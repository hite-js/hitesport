<html lang="en">
  ...
  <?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        if($_POST['oldpass'] == $_POST['oldpass_verify']){
            if(md5($_POST['oldpass']) == $user['password']){
                $password = md5($_POST['password']);
                $changepass = "UPDATE users SET password='$password' WHERE username='$username'";
                $result = $mysqli->query($changepass) or die($mysqli->error);
            ...
            $_SESSION['visibility'] = 'visible';
            $_SESSION['message'] = 'The two old passwords do not match';
        }
    }
...
<div class="form-container">
  <div class="form-signin">
    <form action="changepass.php" method="post" autocomplete="off">
      ...
      <label for="inputOldPassword" class="visually-hidden">Old Password</label>
      <input type="password" name="oldpass" id="inputoldpass" class="form-control" placeholder="Old Password*" required autofocus>
      <label for="inputOldPasswordVerify" class="visually-hidden">Old Password</label>
      <input type="password" name="oldpass_verify" id="inputoldpassverify" class="form-control" placeholder="Repeat Old Password*" required autofocus>
      <label for="inputPassword" class="visually-hidden">Password</label>
      <input type="password" name="password" id="inputPassword" class="form-control" placeholder="New Password*" required>
      <button class="btn btn-lg btn-warning btn-block" type="submit">Change password</button>     
    </form>
  </div>
</div>
<!--BODY END-->
